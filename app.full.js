...
function nav(e) {
	...
	n += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${o}</a><div class="dropdown-menu" aria-labelledby="navbarDropdown">`, i.forEach(((e, t) => {
		n += `<a class="dropdown-item"  href="/${t}:/">${e}</a>`
	})), n += "</div></li>", n += `<li class="nav-item">\n    <a class="nav-link" href="${UI.contact_link}" target="_blank">${UI.nav_link_4}</a>\n  </li>${UI.show_logout_button?'<li class="nav-item"><a class="nav-link" href="/logout">Logout</a></li>':""}`;
...
function requestListPath(e, t, n, a, i = 3, o = !1) {
	...
	$("#update").show(), document.getElementById("update").innerHTML = "<div class='alert alert-info' role='alert'> Connecting...</div></div></div>", o && (e = "/0:fallback"), console.log("Performing Request again"),
		function t() {
			fetch(o ? "/0:fallback" : e, {
				...
				t && t.error && 401 === t.error.code ? askPassword(e) : t && null === t.data ? (document.getElementById("spinner").remove(), document.getElementById("list").innerHTML = "<div class='alert alert-danger' role='alert'> Server didn't send any data.</div></div></div>", $("#update").hide()) : t && t.data && (n(t, e, l), $("#update").hide())
			})).catch((function(e) {
				i > 0 ? (sleep(2e3), document.getElementById("update").innerHTML = "<div class='alert alert-info' role='alert'> Retrying...</div></div></div>", t()) : (document.getElementById("update").innerHTML = "<div class='alert alert-danger' role='alert'> Unable to get data from the server. Something went wrong.</div></div></div>", document.getElementById("list").innerHTML = "<div class='alert alert-danger' role='alert'> We were unable to get data from the server. " + e + "</div></div></div>", $("#update").hide())
			}))
		}()
}
	...
	$("#update").html("<div class='alert alert-info' role='alert'> Connecting...</div></div></div>"),
		function e(n) {
			fetch(`/${window.current_drive_order}:search`, {
				...
				e && null === e.data && ($("#spinner").remove(), $("#list").html("<div class='alert alert-danger' role='alert'> Server didn't send any data.</div></div></div>"), $("#update").remove()), e && e.data && (t && t(e, a), $("#update").remove())
			})).catch((function(t) {
				n > 0 ? (sleep(2e3), $("#update").html("<div class='alert alert-info' role='alert'> Retrying...</div></div></div>"), e(n - 1)) : ($("#update").html("<div class='alert alert-danger' role='alert'> Unable to get data from the server. Something went wrong. 3 Failures</div></div></div>"), $("#list").html("<div class='alert alert-danger' role='alert'> We were unable to get data from the server.</div></div></div>"), $("#spinner").remove())
			}))
		}(n)
}
...
function list(e, t = "", n = !1) {
	...
	function p(e, a, i) {
		console.log(e, a, i), $("#list").data("nextPageToken", e.nextPageToken).data("curPageIndex", e.curPageIndex), $("#spinner").remove(), null === e.nextPageToken ? ($(window).off("scroll"), window.scroll_status.event_bound = !1, window.scroll_status.loading_lock = !1, n ? append_files_to_fallback_list(a, e.data.files) : append_files_to_list(a, e.data.files)) : (console.log("doing something..."), n ? append_files_to_fallback_list(a, e.data.files) : append_files_to_list(a, e.data.files), !0 !== window.scroll_status.event_bound && ($(window).on("scroll", (function() {
			var e = $(this).scrollTop(),
				o = getDocumentHeight();
			if(e + $(this).height() > o - (Os.isMobile ? 130 : 80)) {
				if(!0 === window.scroll_status.loading_lock) return;
				window.scroll_status.loading_lock = !0, $(`<div id="spinner" class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`).insertBefore("#readme_md");
				let e = $("#list");
				n ? (console.log("fallback inside handleSuccessResult"), requestListPath(a, {
					id: t,
					password: i.password,
	...
	$("#list").html(`<div class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`), $("#readme_md").hide().html(""), $("#head_md").hide().html(""), n ? (console.log("fallback inside list"), requestListPath(e, {
		id: t,
		password: c
	}, p, null, null, n = !0)) : (console.log("handling this"), requestListPath(e, {
		password: c
	}, p, null));
	document.getElementById("handle-multiple-items-copy").addEventListener("click", (() => {
		const e = document.querySelectorAll('input[type="checkbox"]:checked'),
			t = [];
		if(0 === e.length) return void alert("No items selected!");
		e.forEach((e => {
			const n = e.value;
			t.push(n)
		}));
		...
		a.value = n, document.body.appendChild(a), a.select(), document.execCommand("copy"), document.body.removeChild(a), alert("Selected items copied to clipboard!")
	}))
}
...
function append_files_to_fallback_list(e, t) {
	try {
		...
		for(i in t) {
			var s = t[i],
				d = "/fallback?id=" + s.id;
			if(s.modifiedTime = utc2delhi(s.modifiedTime), "application/vnd.google-apps.folder" == s.mimeType) html += `<a href="${d}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${s.name} ${UI.display_time?'<span class="badge bg-info float-end"> '+s.modifiedTime+" </span>":""}</a>`;
			else {
				l = l + Number(s.size);
				...
				"?a=view", " view", html += '<div class="list-group-item list-group-item-action">' + (UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' + p + '" id="flexCheckDefault">' : ""), "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${m}|`) >= 0 ? html += video_icon : "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${m}|`) >= 0 ? html += code_icon : "|zip|rar|tar|.7z|.gz|".indexOf(`|${m}|`) >= 0 ? html += zip_icon : "|bmp|jpg|jpeg|png|gif|".indexOf(`|${m}|`) >= 0 ? html += image_icon : "|m4a|mp3|flac|wav|ogg|".indexOf(`|${m}|`) >= 0 ? html += audio_icon : "|md|".indexOf(`|${m}|`) >= 0 ? html += markdown_icon : "|pdf|".indexOf(`|${m}|`) >= 0 ? html += pdf_icon : html += file_icon, html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${d}&a=view">${s.name}</a>${UI.display_download?` < a href = "${p}" > < svg class = "float-end"
				width = "25px"
				...
				height = "16"
				fill = "currentColor"
				viewBox = "0 0 16 16" > < path d = "M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z" > < /path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path > < /svg></a > `:""}${UI.display_size?'<span class="badge bg-primary float-end"> '+s.size+" </span>":""}${UI.display_time?' <span class="badge bg-info float-end"> '+s.modifiedTime+" </span>":""}</div>`
			}
		}
		if(r && UI.allow_selecting_files && (document.getElementById("select_items").style.display = "block"), g.length > 0) {
		...
		n.html(("0" == n.data("curPageIndex") ? "" : n.html()) + html), a && (total_size = formatFileSize(l) || "0 Bytes", total_items = n.find(".countitems").length, total_files = n.find(".size_items").length, 0 == total_items ? $("#count").removeClass("d-none").find(".number").text("Empty Folder") : 1 == total_items ? $("#count").removeClass("d-none").find(".number").text(total_items + " item") : $("#count").removeClass("d-none").find(".number").text(total_items + " items"), 0 == total_files ? $("#count").removeClass("d-none").find(".totalsize").text("Zero Files") : 1 == total_files ? $("#count").removeClass("d-none").find(".totalsize").text(total_files + " File with Size " + total_size) : $("#count").removeClass("d-none").find(".totalsize").text(total_files + " Files with Size " + total_size))
	} catch(e) {
		console.log(e)
	}
}
...
function append_files_to_list(e, t) {
	...
	for(i in t) {
		...
		if(d.modifiedTime = utc2delhi(d.modifiedTime), "application/vnd.google-apps.folder" == d.mimeType) html += `<a href="${c}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${d.name} ${UI.display_time?'<span class="badge bg-info float-end"> '+d.modifiedTime+" </span>":""}</a>`;
		else {
			r = r + Number(d.size);
			...
			console.log(f), g += "?a=view", " view", html += '<div class="list-group-item list-group-item-action">' + (UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' + m + '" id="flexCheckDefault">' : ""), "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${f}|`) >= 0 ? html += video_icon : "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${f}|`) >= 0 ? html += code_icon : "|zip|rar|tar|.7z|.gz|".indexOf(`|${f}|`) >= 0 ? html += zip_icon : "|bmp|jpg|jpeg|png|gif|".indexOf(`|${f}|`) >= 0 ? html += image_icon : "|m4a|mp3|flac|wav|ogg|".indexOf(`|${f}|`) >= 0 ? html += audio_icon : "|md|".indexOf(`|${f}|`) >= 0 ? html += markdown_icon : "|pdf|".indexOf(`|${f}|`) >= 0 ? html += pdf_icon : html += file_icon, html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${g}">${d.name}</a>${UI.display_download?` < a href = "${m}" > < svg class = "float-end"
			width = "25px"
			style = "margin-left: 8px;"
			xmlns = "http://www.w3.org/2000/svg"
			width = "16"
			height = "16"
			fill = "currentColor"
			viewBox = "0 0 16 16" > < path d = "M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z" > < /path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path > < /svg></a > `:""}${UI.display_size?'<span class="badge bg-primary float-end"> '+d.size+" </span>":""}${UI.display_time?' <span class="badge bg-info float-end"> '+d.modifiedTime+" </span>":""}</div>`
		}
	}
	if(s && UI.allow_selecting_files && (document.getElementById("select_items").style.display = "block"), l.length > 0) {
		let t = localStorage.getItem(e),
			n = l;
		if(!o && t) {
			let e;
	...
	n.html(("0" == n.data("curPageIndex") ? "" : n.html()) + html), a && (total_size = formatFileSize(r) || "0 Bytes", total_items = n.find(".countitems").length, total_files = n.find(".size_items").length, 0 == total_items ? $("#count").removeClass("d-none").find(".number").text("Empty Folder") : 1 == total_items ? $("#count").removeClass("d-none").find(".number").text(total_items + " item") : $("#count").removeClass("d-none").find(".number").text(total_items + " items"), 0 == total_files ? $("#count").removeClass("d-none").find(".totalsize").text("Zero Files") : 1 == total_files ? $("#count").removeClass("d-none").find(".totalsize").text(total_files + " File with Size " + total_size) : $("#count").removeClass("d-none").find(".totalsize").text(total_files + " Files with Size " + total_size))
}
...
function render_search_result_list() {
	...
	$("#content").html(e), $("#list").html(`<div class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`), $("#readme_md").hide().html(""), $("#head_md").hide().html(""), requestSearch({
		q: window.MODEL.q
	}, (function e(t, n) {
...
function append_search_result_to_list(e) {
	try {
		...
		for(i in e) {
			var l = e[i];
			if(null == l.size && (l.size = ""), l.modifiedTime = utc2delhi(l.modifiedTime), "application/vnd.google-apps.folder" == l.mimeType) html += `<a style="color: ${UI.folder_text_color};" onclick="onSearchResultItemClick('${l.id}', false)" data-bs-toggle="modal" data-bs-target="#SearchModel" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${l.name} ${UI.display_time?'<span class="badge bg-info float-end"> '+l.modifiedTime+" </span>":""}</a>`;
			else {
				o = !0, a = a + Number(l.size);
				...
				html += `<div style="color: ${UI.css_a_tag_color};" gd-type="$item['mimeType']}" class="countitems size_items list-group-item list-group-item-action">${UI.allow_selecting_files?'<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="'+s+'" id="flexCheckDefault">':""}`, "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${r}|`) >= 0 ? html += video_icon : "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${r}|`) >= 0 ? html += code_icon : "|zip|rar|tar|.7z|.gz|".indexOf(`|${r}|`) >= 0 ? html += zip_icon : "|bmp|jpg|jpeg|png|gif|".indexOf(`|${r}|`) >= 0 ? html += image_icon : "|m4a|mp3|flac|wav|ogg|".indexOf(`|${r}|`) >= 0 ? html += audio_icon : "|md|".indexOf(`|${r}|`) >= 0 ? html += markdown_icon : "|pdf|".indexOf(`|${r}|`) >= 0 ? html += pdf_icon : html += file_icon, html += ` <span onclick="onSearchResultItemClick('${l.id}', true)" data-bs-toggle="modal" data-bs-target="#SearchModel">${l.name}</span>${UI.display_download?` < a href = "${s}" > < svg class = "float-end"
				width = "25px"
				style = "margin-left: 8px;"
				xmlns = "http://www.w3.org/2000/svg"
				width = "16"
				height = "16"
				fill = "currentColor"
				viewBox = "0 0 16 16" > < path d = "M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z" > < /path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path > < /svg></a > `:""}<span class="badge float-end csize"> ${UI.display_size?'<span class="badge bg-primary float-end"> '+l.size+" </span>":""}${UI.display_time?' <span class="badge bg-info float-end"> '+l.modifiedTime+" </span>":""}</div>`
			}
		}
		o && UI.allow_selecting_files && (document.getElementById("select_items").style.display = "block"), t.html(("0" == t.data("curPageIndex") ? "" : t.html()) + html), n && (total_size = formatFileSize(a) || "0 Bytes", total_items = t.find(".countitems").length, total_files = t.find(".size_items").length, 0 == total_items ? $("#count").removeClass("d-none").find(".number").text("No Results") : 1 == total_items ? $("#count").removeClass("d-none").find(".number").text(total_items + " item") : $("#count").removeClass("d-none").find(".number").text(total_items + " items"), 0 == total_files ? $("#count").removeClass("d-none").find(".totalsize").text("Found Nothing") : 1 == total_files ? $("#count").removeClass("d-none").find(".totalsize").text(total_files + " File with Size " + total_size) : $("#count").removeClass("d-none").find(".totalsize").text(total_files + " Files with Size " + total_size))
	} catch(e) {
		console.log(e)
	}
}
...
function onSearchResultItemClick(e, t) {
	...
	fetch(`/${n}:id2path`, {
		...
		a = "Result", $("#SearchModelLabel").html(a), i = `<a class="btn btn-info" href="${n}${t?"?a=view":""}">Open</a> <a class="btn btn-secondary" href="${n}${t?"?a=view":""}" target="_blank">Open in New Tab</a>`, $("#modal-body-space").html(i)
	})).catch((function(n) {
		console.log(n);
		a = "Fallback Method", $("#SearchModelLabel").html(a), i = `<a class="btn btn-info" href="/fallback?id=${e}&${t?"a=view":""}">Open</a> <a class="btn btn-secondary" href="/fallback?id=${e}&${t?"a=view":""}" target="_blank">Open in New Tab</a>`, $("#modal-body-space").html(i)
	}))
}
...
async function fallback(e, t) {
	if(!t) return list(e, !0);
	var n = await getCookie("root_id") || "";
	$("#content").html(`<div class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`), fetch("/0:fallback", {
		method: "POST",
		headers: {
			"Content-Type": "application/json"
		},
		...
		const i = ["php", "css", "go", "java", "js", "json", "txt", "sh", "md", "html", "xml", "py", "rb", "c", "cpp", "h", "hpp"],
			...
			s = ["pdf"];
		if("application/vnd.google-apps.folder" === t) window.location.href = window.location.pathname + "/";
		else if(a) {
			const d = e.name,
			...
			} else t.includes("audio") || l.includes(a) ? file_audio(d, c, p, m, g, n) : t.includes("image") || r.includes(a) ? file_image(d, c, p, m, g, n) : t.includes("pdf") || s.includes(a) ? file_pdf(d, c, p, m, g, n) : i.includes(a) ? file_code(d, c, p, m, g, n) : file_others(d, c, p, m, g, n)
		}
	})).catch((function(e) {
...
async function file(e) {
	...
	e.split("/").pop();
	$("#content").html(`<div class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`), fetch("", {
		method: "POST",
		headers: {
			"Content-Type": "application/json"
		},
		...
		const i = ["php", "css", "go", "java", "js", "json", "txt", "sh", "md", "html", "xml", "py", "rb", "c", "cpp", "h", "hpp"],
			...
			r = ["bmp", "jpg", "jpeg", "png", "gif", "svg", "tiff", "ico"],
			s = ["pdf"];
		if("application/vnd.google-apps.folder" === n) window.location.href = window.location.pathname + "/";
		else if(a) {
			const d = e.name,
				c = encodeURIComponent(d),
				p = formatFileSize(e.size),
			...
			} else n.includes("audio") || l.includes(a) ? file_audio(d, c, p, m, g, t) : n.includes("image") || r.includes(a) ? file_image(d, c, p, m, g, t) : n.includes("pdf") || s.includes(a) ? file_pdf(d, c, p, m, g, t) : i.includes(a) ? file_code(d, c, p, m, g, t) : file_others(d, c, p, m, g, t)
		}
	})).catch((function(e) {
		var t = `\n          <div class="container"><br>\n          <div class="card text-center">\n            <div class="card-body text-center">\n              <div class="${UI.file_view_alert_class}" id="file_details" role="alert"><b>404.</b> That’s an error. ` + e + '</div>\n            </div>\n            <p>The requested URL was not found on this server. That’s all we know.</p>\n            <div class="card-text text-center">\n              <div class="btn-group text-center">\n                <a href="/" type="button" class="btn btn-primary">Homepage</a>\n              </div>\n            </div><br>\n          </div>\n        </div>';
		$("#content").html(t)
...
function file_others(e, t, n, a, i, o) {
	...
	for(var s = "", d = "", c = 0; c < l.length; c++) {
		var p = l[c];
		c == l.length - 1 ? d += p + "?a=view" : d += p + "/", p.length > 15 && (p = (p = decodeURIComponent(p)).substring(0, 10) + "..."), "" == p && (p = "Home"), s += '<a href="' + d + '" class="breadcrumb-item">' + p + "</a>"
	}
...
function file_code(e, t, n, a, i, o, l, r) {
	...
	for(var c = "", p = "", m = 0; m < s.length; m++) {
		var g = s[m];
		m == s.length - 1 ? p += g + "?a=view" : p += g + "/", g.length > 15 && (g = (g = decodeURIComponent(g)).substring(0, 10) + "..."), "" == g && (g = "Home"), c += '<a href="' + p + '" class="breadcrumb-item">' + g + "</a>"
	}
	...
	$("#content").html(f);
	$("#code_spinner").html('<div class="d-flex justify-content-center"><div class="spinner-border m-5" role="status"><span class="sr-only"></span></div></div>'), a <= 2097152 ? $.get(i, (function(e) {
		$("#editor").html($("<div/>").text(e).html()), $("#code_spinner").html("")
	})) : ($("#code_spinner").html(""), $("#editor").html(`<div class="${UI.file_view_alert_class}" id="file_details" role="alert">File size is too large to preview, Max Limit is 2 MB</div>`))
}
...
function file_video(e, t, n, a, i, o, l, r) {
	...
	for(var p = "", m = "", g = 0; g < d.length; g++) {
		var f = d[g];
		g == d.length - 1 ? m += f + "?a=view" : m += f + "/", f.length > 15 && (f = (f = decodeURIComponent(f)).substring(0, 10) + "..."), "" == f && (f = "Home"), p += '<a href="' + m + '" class="breadcrumb-item">' + f + "</a>"
	}
	let v;
	UI.disable_player || ("plyr" == player_config.player ? (v = `<video id="player" playsinline controls data-poster="${a}">\n      <source src="${i}" type="video/mp4" />\n      <source src="${i}" type="video/webm" />\n        </video>`, player_js = "https://cdn.plyr.io/" + player_config.plyr_io_version + "/plyr.polyfilled.js", player_css = "https://cdn.plyr.io/" + player_config.plyr_io_version + "/plyr.css") : "videojs" == player_config.player ? (v = `<video id="vplayer" poster="${a}" muted=true class="video-js vjs-default-skin" controls preload="auto" width="100%" height="100%" data-setup='{"fluid": true}' style="--plyr-captions-text-color: #ffffff;--plyr-captions-background: #000000;">\n      <source src="${i}" type="video/mp4" />\n      <source src="${i}" type="video/webm" />\n      <source src="${i}" type="video/avi" />\n    </video>`, player_js = "https://vjs.zencdn.net/" + player_config.videojs_version + "/video.js", player_css = "https://vjs.zencdn.net/" + player_config.videojs_version + "/video-js.css") : "dplayer" == player_config.player ? (v = '<div id="player-container"></div>', player_js = "https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.js", player_css = "https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.css") : "jwplayer" == player_config.player && (v = '<div id="player"></div>', player_js = "https://content.jwplatform.com/libraries/IDzF9Zmk.js", player_css = ""));
	...
	href = "iina://weblink?url=${i}" > IINA < /a>\n              <a class="dropdown-item" href="potplayer:/ / $ {
		i
	}
	">PotPlayer</a>\n              <a class="
	dropdown - item " href="
	vlc: //${i}">VLC Mobile</a>\n              <a class="dropdown-item" href="${i}">VLC Desktop</a>\n              <a class="dropdown-item" href="nplayer-${i}">nPlayer</a>\n              <a class="dropdown-item" href="intent://${i}#Intent;type=video/any;package=is.xyz.mpv;scheme=https;end;">mpv-android</a>\n              <a class="dropdown-item" href="mpv://${s}">mpv x64</a>\n              <a class="dropdown-item" href="intent:${i}#Intent;package=com.mxtech.videoplayer.ad;S.title=${t};end">MX Player (Free)</a>\n              <a class="dropdown-item" href="intent:${i}#Intent;package=com.mxtech.videoplayer.pro;S.title=${t};end">MX Player (Pro)</a>\n              <a class="dropdown-item" href="intent:${i}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Free)</a>\n              <a class="dropdown-item" href="intent:${i}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Lite)</a>\n              <a class="dropdown-item" href="intent:${i}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${t};end">1DM+ (Plus)</a>\n              </div>\n          </div>\n          `+copyButton+c+"\n          \n          </div>\n          </div>\n          "}\n      </div>\n    </div>\n  `;$("#content").html(u);var h=document.createElement("script");h.src=player_js,h.onload=function(){if("plyr"==player_config.player){new Plyr("#player")}else if("videojs"==player_config.player){new videojs("vplayer")}else if("dplayer"==player_config.player){new DPlayer({container:document.getElementById("player-container"),screenshot:!0,video:{url:i,pic:a,thumbnails:a}})}else"jwplayer"==player_config.player&&jwplayer("player").setup({file:i,type:o,autostart:!1,image:a,width:"100%",aspectratio:"16:9",title:e,description:"Powered by Google Drive Index",tracks:[{file:i,kind:"captions",label:"Default",default:!0}],captions:{color:"#f3f378",fontSize:14,backgroundOpacity:50,edgeStyle:"raised"}})},document.head.appendChild(h);var y=document.createElement("link");y.href=player_css,y.rel="stylesheet",document.head.appendChild(y)}function file_audio(e,t,n,a,i,o){var l=btoa(a),r=window.location.pathname.split("/");const s=UI.allow_file_copy?generateCopyFileBox(i,o):"";for(var d="",c="",p=0;p<r.length;p++){var m=r[p];p==r.length-1?c+=m+"?a=view":c+=m+"/",m.length>15&&(m=(m=decodeURIComponent(m)).substring(0,10)+"..."),""==m&&(m="Home"),d+='<a href="'+c+'" class="breadcrumb-item">'+m+"</a>"}var g=`\n    <div class="container text-center"><br>\n      <nav aria-label="breadcrumb">\n        <ol class="breadcrumb">\n          ${d}\n        </ol>\n      </nav>\n      <div class="card text-center">\n        <div class="text-center">\n          <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${e}<br>${n}</div>\n          ${UI.disable_player?"":`\n          <video id="aplayer" poster="${UI.audioposter}" muted=true class="video-js vjs-default-skin" controls preload="auto" width="100%" height="100%" data-setup='{"fluid": true}' style="--plyr-captions-text-color: #ffffff;--plyr-captions-background: #000000;">\n            <source src="${a}" type="audio/mpeg" />\n            <source src="${a}" type="audio/ogg" />\n            <source src="${a}" type="audio/wav" />\n          </video>`}\n        </div>\n        </br>\n        ${UI.disable_audio_download?"":`\n          <div class="card-body">\n          <div class="input-group mb-4">\n          <input type="text" class="form-control" id="dlurl" value="${a}" readonly>\n          </div>\n          <div class="btn-group text-center">\n              <a href="${a}" type="button" class="btn btn-primary">Download</a>\n              <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">\n              <span class="sr-only"></span>\n              </button>\n              <div class="dropdown-menu">\n              <a class="dropdown-item" href="iina://weblink?url=${a}">IINA</a>\n              <a class="dropdown-item" href="potplayer://${a}">PotPlayer</a>\n              <a class="dropdown-item" href="vlc://${a}">VLC Mobile</a>\n              <a class="dropdown-item" href="${a}">VLC Desktop</a>\n              <a class="dropdown-item" href="nplayer-${a}">nPlayer</a>\n              <a class="dropdown-item" href="intent://${a}#Intent;type=audio/any;package=is.xyz.mpv;scheme=https;end;">mpv-android</a>\n              <a class="dropdown-item" href="mpv://${l}">mpv x64</a>\n              <a class="dropdown-item" href="intent:${a}#Intent;package=com.mxtech.videoplayer.ad;S.title=${t};end">MX Player (Free)</a>\n              <a class="dropdown-item" href="intent:${a}#Intent;package=com.mxtech.videoplayer.pro;S.title=${t};end">MX Player (Pro)</a>\n              <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Free)</a>\n              <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Lite)</a>\n              <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${t};end">1DM+ (Plus)</a>\n              </div>\n          </div>\n          `+copyButton+s+"\n          <br>\n          </div>\n          </div>\n          "}\n      </div>\n    </div>\n  `;$("#content").html(g);var f=document.createElement("script");f.src="https://vjs.zencdn.net/"+UI.videojs_version+"/video.min.js",f.onload=function(){videojs("aplayer")},document.head.appendChild(f);var v=document.createElement("link");v.href="https://vjs.zencdn.net/"+UI.videojs_version+"/video-js.css",v.rel="stylesheet",document.head.appendChild(v)}function file_pdf(e,t,n,a,i,o){var l=window.location.pathname.split("/");UI.allow_file_copy&&generateCopyFileBox(i,o);for(var r="",s="",d=0;d<l.length;d++){var c=l[d];d==l.length-1?s+=c+"?a=view":s+=c+"/",c.length>15&&(c=(c=decodeURIComponent(c)).substring(0,10)+"..."),""==c&&(c="Home"),r+='<a href="'+s+'" class="breadcrumb-item">'+c+"</a>"}var p=`\n    <div class="container text-center"><br>\n      <nav aria-label="breadcrumb">\n        <ol class="breadcrumb">\n          ${r}\n        </ol>\n      </nav>\n    <div class="card">\n    <div class="card-body text-center">\n    <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${e}<br>${n}</div>\n    <div>\n    </div><br>\n    <iframe src="https://docs.google.com/gview?url=${encodeURIComponent(a)}&embedded=true" style="width:100%; height:500px;" frameborder="0"></iframe>\n    </div>\n    <div class="card-body">\n    <div class="input-group mb-4">\n    <input type="text" class="form-control" id="dlurl" value="${a}" readonly>\n    </div>\n    <div class="card-text text-center">\n    <div class="btn-group text-center">\n        <a href="${a}" type="button" class="btn btn-primary">Download</a>\n        <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">\n            <span class="sr-only"></span>\n        </button>\n        <div class="dropdown-menu">\n            <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Free)</a>\n            <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Lite)</a>\n            <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${t};end">1DM+ (Plus)</a>\n        </div>\n    </div>\n    `+copyButton+"\n    </div>\n    <br>\n    \n    </div>\n    </div>\n    </div>  \n  ";$("#content").html(p)}function file_image(e,t,n,a,i,o){var l=window.location.pathname.split("/");const r=UI.allow_file_copy?generateCopyFileBox(i,o):"";for(var s="",d="",c=0;c<l.length;c++){var p=l[c];c==l.length-1?d+=p+"?a=view":d+=p+"/",p.length>15&&(p=(p=decodeURIComponent(p)).substring(0,10)+"..."),""==p&&(p="Home"),s+='<a href="'+d+'" class="breadcrumb-item">'+p+"</a>"}var m=`\n    <div class="container text-center"><br>\n      <nav aria-label="breadcrumb">\n        <ol class="breadcrumb">\n          ${s}\n        </ol>\n      </nav>\n      <div class="card">\n        <div class="card-body text-center">\n          <div class="${UI.file_view_alert_class}" id="file_details" role="alert">${e}<br>${n}</div>\n          <img src="${a}" id="load_image" width="100%">\n        </div>\n        <div class="card-body">\n          <div class="input-group mb-4">\n            <div class="input-group-prepend">\n              <span class="input-group-text" id="">Full URL</span>\n            </div>\n            <input type="text" class="form-control" id="dlurl" value="${a}" readonly>\n          </div>\n          <div class="card-text text-center">\n            <div class="btn-group text-center">\n              <a href="${a}" type="button" class="btn btn-primary">Download</a>\n              <button type="button" class="btn btn-primary dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">\n                <span class="sr-only"></span>\n              </button>\n              <div class="dropdown-menu">\n                <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Free)</a>\n                <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager.adm.lite/idm.internet.download.manager.Downloader;S.title=${t};end">1DM (Lite)</a>\n                <a class="dropdown-item" href="intent:${a}#Intent;component=idm.internet.download.manager.plus/idm.internet.download.manager.Downloader;S.title=${t};end">1DM+ (Plus)</a>\n              </div>\n            </div>\n            `+copyButton+r+"\n          </div>\n          <br>\n        </div>\n      </div>\n    </div>\n  ";$("#content").html(m)}function utc2delhi(e){var t=new Date(e),n=new Date(t.getTime()+198e5),a=n.getFullYear(),i=("0"+(n.getMonth()+1)).slice(-2);return`${("0"+n.getDate()).slice(-2)}-${i}-${a} ${("0"+n.getHours()).slice(-2)}:${("0"+n.getMinutes()).slice(-2)}:${("0"+n.getSeconds()).slice(-2)}`}function formatFileSize(e){return e>=1099511627776?e=(e/1099511627776).toFixed(2)+" TB":e>=1073741824?e=(e/1073741824).toFixed(2)+" GB":e>=1048576?e=(e/1048576).toFixed(2)+" MB":e>=1024?e=(e/1024).toFixed(2)+" KB":e>1?e+=" bytes":1===e?e+=" byte":e="",e}function markdown(e,t){var n=marked.parse(t);$(e).show().html(n)}function copyFunction(){var e=document.getElementById("dlurl");e.select(),e.setSelectionRange(0,99999),navigator.clipboard.writeText(e.value).then((function(){document.getElementById("myTooltip").innerHTML="Copied"})).catch((function(e){console.error("Failed to copy text: ",e)}))}function outFunc(){document.getElementById("myTooltip").innerHTML="Copy"}function updateCheckboxes(){const e=document.querySelectorAll('input[type="checkbox"]'),t=document.getElementById("select-all-checkboxes");e.length>0&&t&&t.addEventListener("click",(()=>{e.forEach((e=>{e.checked=t.checked}))}))}async function getCookie(e){for(var t=e+"=",n=document.cookie.split(";"),a=0;a<n.length;a++){for(var i=n[a];" "==i.charAt(0);)i=i.substring(1,i.length);if(0==i.indexOf(t))return i.substring(t.length,i.length)}return null}async function copyFile(e){try{const t=document.getElementById("copystatus");t.innerHTML="<div class='alert alert-danger' role='alert'> Processing... </div>";const n=document.getElementById("user_folder_id").value;if(""===n)return t.innerHTML="<div class='alert alert-danger' role='alert'> Empty ID </div>",null;document.getElementById("spinner").style.display="block",document.cookie=`root_id=${n}; expires=Thu, 18 Dec 2050 12:00:00 UTC`;const a=Math.floor(Date.now()/1e3),i=await fetch("/copy",{method:"POST",cache:"no-cache",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`id=${encodeURIComponent(e)}&root_id=${n}&resourcekey=null&time=${a}`});if(500===i.status)t.innerHTML="<div class='alert alert-danger' role='alert'> Unable to Copy File, Make Sure you've added system@zindex.eu.org to your Destination Folder </div>";else if(401===i.status)t.innerHTML="<div class='alert alert-danger' role='alert'> Unauthorized </div>";else if(i.ok){const e=await i.json();if(e&&e.name){const t=`https://drive.google.com/file/d/${e.id}/view?usp=share_link`;document.getElementById("copyresult").innerHTML=`<div class="col-12 col-md-12"> <input type="text" id="usercopiedfile" class="form-control" placeholder="Enter Your Folder ID to Copy this File" value="${t}" readonly></div> <div class="col-12 col-md-12"> <a href="${t}" target="_blank" style="margin-top: 5px;" class="btn btn-danger btn-block">Open Copied File</a></div>`}else e&&e.error&&e.error.message?t.innerHTML="<div class='alert alert-danger' role='alert'> "+e.error.message+" </div>":t.innerHTML="<div class='alert alert-danger' role='alert'> Unable to Copy File </div>"}else t.innerHTML="<div class='alert alert-danger' role='alert'> Unable to Copy File </div>";document.getElementById("spinner").style.display="none"}catch(e){document.getElementById("copystatus").innerHTML="<div class='alert alert-danger' role='alert'> An error occurred "+e+"</div>",document.getElementById("spinner").style.display="none"}}String.prototype.trim=function(e){return e?this.replace(new RegExp("^\\"+e+"+|\\"+e+"+$","g"),""):this.replace(/^\s+|\s+$/g,"")},window.onpopstate=function(){render(window.location.pathname)},$((function(){init(),render(window.location.pathname)}));const observer=new MutationObserver((()=>{updateCheckboxes()})),options={childList:!0,subtree:!0};observer.observe(document.documentElement,options);
		//# sourceMappingURL=/sm/08240548316c1f770d64712c5163ce5fdc86f3990766b4c8da6660f5b52a9955.map
